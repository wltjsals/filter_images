...
// 이미지 요소 배열을 순회한다. 첫번째 파라미터는 이미지 요소 배열, 두번째 파라미터는 boolean 값 - true일 경울 ? 뒤에 : 을  기준으로 앞에 로직이 적용되고, false 일 경우 뒤에 로직이 적용된다.
...
function showItems(thisItem) {
  ...
  const thisImage = Array.from(images).filter(
    //Array,from() : 유사배열(NodeList, HTMLcollection
...
btns.forEach((btn) => {
  ...
  btn.addEventListener("click", function () {
    ...
    // 이미지 요소들이 재배치 될 때 0.1초 뒤에 showItem 함수 실행
    setTimeout(() => {
      showItems(this);
    }, 100);
  });
...
overlay.addEventListener("click", closeLightBox); // 베경 오버레이를 클릭했을 때 함수 실행
